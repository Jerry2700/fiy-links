...
const Dp = () => {
    ...
    axios
      ...
      .then((response) => {
        ...
        if(response.data.image){
          setImage("http://192.168.0.108:8080/user/auth/get-user-image/"+response.data.image);
        }
        else{
          setImage(img);
  ...
  const handleupdate=(link)=>{
    ...
    axios.put("http://192.168.0.108:8080/user/auth/update-visited",obj )
    .then((response) => {
      console.log(response);
    })
    .catch(error => {
    ...
    <>
      ...
      >
        ...
        >
          ...
          {productdata.map((link, index) => (
                ...
                <div key={index} className="card" style={{width:'auto',maxWidth:'14rem'}}>
                  <a href={link.productUrl} style={{margin:'0',padding:'0'}} onClick={() => handleupdateproduct(link)} >
                    <div className="product-image">
            <img src={(link.productImage) ? ("http://192.168.0.108:8080/user/auth/get-product-image/" + link.productImage) : ('dsad')} className="card-img-top" alt="cant loaded"/>  </div>
            </a>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <h5 class="card-title" style={{margin:'0 0 0 1vw',display:'inline'}} >{link.productName}</h5>
      ...
      >
        ...
        <DialogContent>
          ...
          <iframe src={link.productVideo} style={{height:'50vh',width:'30vw'}}>
                    </iframe>
          </DialogContentText>
        </DialogContent>
